...
test('collects only bounded health evidence into a secret-free incident record', async () => {
  ...
  const evidenceDir = await mkdtemp(join(tmpdir(), 'paircode-evidence-'));
  await writeFile(join(evidenceDir, 'metrics.json'), JSON.stringify({ errors: 1, token: 'secret' }));
  await writeFile(join(evidenceDir, 'logs.json'), 'failed https://example.test');
  ...
  assert.equal(record.evidence.find((item) => item.source === 'metrics').data.token, '[REDACTED]');
  assert.equal(record.evidence.find((item) => item.source === 'logs').data, 'failed [REDACTED]');
  server.close();
});
